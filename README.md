...
[![Version](https://img.shields.io/badge/version-1.62.0-blue.svg)](https://github.com/whoishzp/magicer/releases)
...
- **启动命令**：App 启动时自动执行配置的 Shell 命令（可配置多条，各自启用/禁用）；每条可一键立即执行；**执行记录**（含启动/手动、时间、成功失败、退出码）本地持久化最多 50 条；手动执行结束后显示柔和提示条（约 6 秒后淡出）
...
│   │   └── StartupCommandLogEntry.swift # 启动命令执行记录
│   ├── Timer/
