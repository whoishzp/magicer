...
[![Version](https://img.shields.io/badge/version-1.65.0-blue.svg)](https://github.com/whoishzp/magicer/releases)
...
- **启动命令**：App 启动时自动执行配置的 Shell 命令（可配置多条，各自启用/禁用）；每条可一键复制完整 shell 到剪贴板（复制成功有约 3 秒柔和提示）、一键立即执行；手动执行结束后显示柔和提示条（约 6 秒后淡出，含退出码或错误摘要）
